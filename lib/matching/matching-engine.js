...
import { aiMatch, isAIEnabled } from './ai-hook.js';
...
class MatchingEngine {
    ...
    async findAnswer(query, options = {}) {
        ...
        try {
            ...
            if (opts.useCache) {
                ...
                if (cachedResult) {
                    timer.end();
                    console.log(`[MatchingEngine] Cache hit (${timer.getDuration().toFixed(2)}ms)`);
                    return cachedResult;
                }
            }
            ...
            const result = await this.runMatchingPipeline(normalizedQuery, keywords, opts);
            ...
            console.log(`[MatchingEngine] Query processed in ${result.processingTime.toFixed(2)}ms`);
    ...
    validateQuery(query) {
        ...
        if (trimmed.length < MATCHING_CONFIG.MIN_QUERY_LENGTH) {
            throw new AppError(
                ERROR_CODES.QUERY_EMPTY,
                'Query is too short',
                { length: trimmed.length, minLength: MATCHING_CONFIG.MIN_QUERY_LENGTH }
            );
        }
        ...
        if (trimmed.length > MATCHING_CONFIG.MAX_QUERY_LENGTH) {
            throw new AppError(
                ...
                'Query is too long',
                { length: trimmed.length, maxLength: MATCHING_CONFIG.MAX_QUERY_LENGTH }
            );
        }
    }
    ...
    async runMatchingPipeline(normalizedQuery, keywords, options) {
        ...
        if (stats.totalQuestions === 0) {
            throw new AppError(
                ERROR_CODES.NO_DATA_LOADED,
                'No answer data loaded. Please upload a Q&A file first.'
            );
        }
        ...
        if (keywords.length > 0) {
            ...
            candidates = await this.dbManager.getQuestionsByKeywords(keywordStrings);
        }

        // If no candidates from keywords, get all questions (expensive!)
        if (candidates.length === 0) {
            console.warn('[MatchingEngine] No keyword candidates, using all questions');
        ...
        if (options.partialEnabled) {
            ...
            if (partialResult && partialResult.confidence >= options.minConfidence) {
                return {
                    success: true,
                    match: partialResult,
                    tier: 4
        ...
        // Tier 5: AI Match (future)
        if (isAIEnabled()) {
            console.log('[MatchingEngine] Tier 5: AI match');
            const aiResult = await aiMatch(normalizedQuery, { keywords, candidates });
            if (aiResult && aiResult.confidence >= options.minConfidence) {
                return {
                    success: true,
                    match: aiResult,
                    tier: 5
        ...
        return {
            success: false,
            match: null,
            message: 'No reliable match found for your query.',
            tier: 0
        };
    }
}
