...
class MatchingEngine {
    ...
    async runMatchingPipeline(normalizedQuery, keywords, options) {
        ...
        if (!hasData) {
            console.log('[MatchingEngine] No local data loaded. Skipping local tiers.');
        }
        ...
        if (aiEnabled) {
            ...
            const aiResult = await aiService.query(options.query, { keywords, candidates });
        ...
        let message = 'No reliable match found for your query.';
        if (!hasData) {
            message = 'No local data loaded and AI execution failed or provided no result.';
        }
