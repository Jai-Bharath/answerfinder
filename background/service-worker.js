...
stateManager.init().then(() => {
    console.log('[ServiceWorker] State manager initialized (v1.2)');
}).catch(error => {
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === 'searchAnswer') {
        ...
        chrome.tabs.sendMessage(tab.id, {
            ...
            console.log('[ServiceWorker] Content script not ready. Attempting to inject...', error.message);
            ...
            try {
                ...
                console.log('[ServiceWorker] Injection successful. Retrying message...');
                ...
                setTimeout(() => {
                    chrome.tabs.sendMessage(tab.id, {
                        type: 'SHOW_ANSWER_OVERLAY',
                        payload: { query: selectedText }
                    }).catch(err => console.error('[ServiceWorker] Retry failed:', err));
                }, 100);

            } catch (injectionError) {
                console.error('[ServiceWorker] Failed to inject content script:', injectionError);
            }
        });
    }
});
